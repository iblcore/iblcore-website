...
const newTestPage = async () => {
  ...
  await page.clock.setFixedTime(`${testDate}T12:00:00Z`);
  return page;
};
...
try {
  ...
  await page.locator('[data-calendar-overview-button]').click();
  await page.locator('button[aria-label^="December 2026,"]').click();
  await page.locator('[data-calendar-month-change="1"]').click();
  assert(await page.locator('[data-calendar-title]').textContent() === "January 2027", "The calendar did not advance into the next year.");
  await page.locator('[data-calendar-overview-button]').click();
  assert(await page.locator('[data-calendar-year-label]').textContent() === "2027", "The month overview did not refresh to the selected year.");
  assert(await page.locator('button[aria-label^="January 2027,"]').count() === 1, "The refreshed overview does not open months in the displayed year.");
  await page.close();
  ...
  const stalePage = await newTestPage();
  ...
  const expectedPast = staleData.filter((event) => event.end_date < testDate).length;
  ...
  const fallbackPage = await newTestPage();
  ...
  const mapFallbackPage = await newTestPage();
  ...
  const arcCard = partnersPage.locator("#profile-issue-146");
  const acneiCard = partnersPage.locator("#profile-issue-236");
  assert(await arcCard.locator("[data-accordion-button]").getAttribute("aria-expanded") === "false", "The ARC profile should start collapsed.");
  assert(await arcCard.locator(".new-partners-card__header .new-partners-person__name").textContent() === "Linda Wilbrecht", "The ARC point person is not shown in the closed Affiliate card.");
  assert(await arcCard.locator(".new-partners-card__header .new-partners-person__image").isVisible(), "The ARC point-person portrait is not shown in the closed Affiliate card.");
  assert(await acneiCard.locator("[data-accordion-button]").getAttribute("aria-expanded") === "false", "The ACNEI profile should start collapsed.");
  assert(await acneiCard.locator(".new-partners-card__header .new-partners-person__name").textContent() === "Alex Gideon", "The ACNEI point person is not shown in the closed Affiliate card.");
  assert(await acneiCard.locator(".new-partners-card__header .new-partners-person__image").isVisible(), "The ACNEI point-person portrait is not shown in the closed Affiliate card.");

  await arcCard.locator("[data-accordion-button]").click();
  assert(await arcCard.getAttribute("data-project-type") === "consortium", "The ARC profile is not classified as a consortium.");
  assert(await arcCard.locator(".new-partners-card__header .new-partners-people").isHidden(), "The ARC point person remains in the expanded-card header.");
  assert(await arcCard.locator(".new-partners-card__panel-people .new-partners-person__name").allTextContents().then((names) => names.includes("Linda Wilbrecht")), "The ARC point person did not move into the expanded people row.");

  await acneiCard.locator("[data-accordion-button]").click();
  assert(await acneiCard.getAttribute("data-project-type") === "consortium", "The ACNEI profile is not classified as a consortium.");
  assert(await acneiCard.locator(".new-partners-card__header .new-partners-people").isHidden(), "The ACNEI point person remains in the expanded-card header.");
  const acneiNames = await acneiCard.locator(".new-partners-card__panel-people .new-partners-person__name").allTextContents();
  assert(JSON.stringify(acneiNames) === JSON.stringify(["Alex Gideon", "Martina Anene-Ogbe", "Ronald Omolo Ouma", "Tolulope Gbayisomore"]), "The expanded ACNEI people row is incomplete or out of order.");
  assert(await acneiCard.locator(".new-partners-card__panel-people .new-partners-person__image").count() === acneiNames.length, "An expanded ACNEI person is missing a portrait.");

  const mathoXiaCard = partnersPage.locator("#profile-issue-134");
  await mathoXiaCard.locator("[data-accordion-button]").click();
  assert(await mathoXiaCard.getAttribute("data-project-type") === "dual-pi", "The Matho-Xia profile is not classified as dual-PI.");
  assert(await mathoXiaCard.locator(".new-partners-card__header .new-partners-person__name").count() === 2, "A dual-PI card did not retain both PIs in its expanded-card header.");
  assert(await mathoXiaCard.locator(".new-partners-card__header .new-partners-people").isVisible(), "The dual-PI people disappeared from the expanded-card header.");

  const buffaloCard = partnersPage.locator("#profile-issue-140");
  await buffaloCard.locator("[data-accordion-button]").click();
  assert(await buffaloCard.getAttribute("data-project-type") === "lab", "The Buffalo profile is not classified as a lab.");
  assert(await buffaloCard.locator(".new-partners-card__header .new-partners-person__name").textContent() === "Elizabeth Buffalo", "A lab card did not retain its PI in the expanded-card header.");
  assert(await buffaloCard.locator(".new-partners-card__header .new-partners-people").isVisible(), "The lab PI disappeared from the expanded-card header.");
  await partnersPage.close();

  const affiliateMapPage = await newTestPage();
  await affiliateMapPage.goto(`${origin}/new-partner-projects/?view=map&filter=affiliate`, { waitUntil: "networkidle" });
  const affiliateFilter = affiliateMapPage.locator('[data-project-filter="affiliate"]');
  assert(await affiliateFilter.getAttribute("aria-pressed") === "true", "The Affiliate URL filter was not selected.");
